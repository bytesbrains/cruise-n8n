...
- A `cru_` project key in n8n's credential store — never in a committed workflow or config file.
- Traffic only to the configured Cruise base URL (production or the demo host).

If a key may have been exposed, revoke it in Cruise and issue a new one. A compromised n8n
instance that held the key has the blast radius of that one key: budget-capped and rate-limited
if it was issued that way.
